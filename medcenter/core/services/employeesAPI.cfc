...
component attributeName='employeesServiceAPI' output='false'{
	...
	function addEmployee( emptName, emptDescription, emptParent, emptChild, emptStatus ){
		...
		result.RETVAL = 0;
		result.RETDESC = "";
		result.STRUCT = structNew(); // дл€ валидации полей

		validator = request.factoryService.getService('Validator');
		                                                 //required                 //DB
		var struct_ = validator.checkInput('#emptName#',true,'isAllowSimbolRusEn',2,50, true, 'type_employees', 'empt_name','Ќаименование типа служащего');
		if ( !struct_.retval ){
	...
	function editeEmployee( emptID, emptName, emptDescription, emptParent, emptChild, emptStatus ){
		...
		validator = request.factoryService.getService('Validator');
		                                                 //required                 //DB
		var struct_ = validator.checkInput('#emptName#',true,'isAllowSimbolRusEn',2,50, false, 'type_employees', 'empt_name','Ќаименование типа служащего');
		if ( !struct_.retval ){
			structInsert(result.struct, 'emptName','#struct_.retdesc#');
		}
