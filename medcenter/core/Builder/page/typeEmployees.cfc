component attributeName='typeEmployees' {
	...
	function Init() {
		...
		widget.menu=request.factoryWidget.GetWidget('menu').Init('typeEmployees');
		widget.info=request.factoryWidget.GetWidget('info').Init( pageName='typeEmployees', sniplet=instance.description);

		widget.employees=request.factoryWidget.GetWidget('employees').init(instance.url.section,instance.url.action);
