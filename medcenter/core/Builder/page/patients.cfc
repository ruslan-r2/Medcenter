component attributeName='patients' {
	...
	function Init() {
		...
		widget.menu=request.factoryWidget.GetWidget('menu').Init('patients');
		widget.info=request.factoryWidget.GetWidget('info').Init(pageName='patients',sniplet=instance.description);
		widget.patients=request.factoryWidget.GetWidget('patients').Init(instance.url.section,instance.url.action);
