component attributeName='users interest' {
	...
	function Init() {
		...
		instance.title='Список пользователей.';

		instance.description='Список пользователей системы.';

		instance.keywords='Список пользователей.';
		...
		widget.menu=request.factoryWidget.GetWidget('menu').Init('usersInterest');
		widget.info=request.factoryWidget.GetWidget('info').Init(pageName='usersInterest',sniplet=instance.description);
		widget.usersInterest=request.factoryWidget.GetWidget('usersInterest').Init(instance.url.section,instance.url.action);
	...
	<BODY>';
		...
		<div class="clear"></div>';
			...
			page &= widget.usersInterest.View();
			page &= '
		<div class="clear"></div>';
