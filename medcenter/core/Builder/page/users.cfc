component attributeName='user list' {
	...
	function Init() {
		...
		widget.menu=request.factoryWidget.GetWidget('menu').Init('users');
		widget.info=request.factoryWidget.GetWidget('info').Init(pageName='users',sniplet=instance.description);
		widget.users=request.factoryWidget.GetWidget('users').Init(instance.url.section,instance.url.action);
