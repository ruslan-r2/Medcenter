component attributeName='service list' {
	...
	function Init() {
		...
		widget.menu=request.factoryWidget.GetWidget('menu').Init('services');
		widget.info=request.factoryWidget.GetWidget('info').Init(pageName='services',sniplet=instance.description);
		widget.services=request.factoryWidget.GetWidget('services').Init(instance.url.section,instance.url.action);
