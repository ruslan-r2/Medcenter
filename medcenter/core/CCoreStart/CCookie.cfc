...
<cfscript>
	...
	function init(){
		...
		for (key in cookie){
			...
			// все что приходит из COOKIES кроме CFID и CFTOKEN нужно удал€ть.
			// JSESSIONID создаетс€ колдой дл€ форм, как отключить или обойтись пока не знаю.
			if ( key != 'CFID' AND key != 'CFTOKEN' AND key != 'JSESSIONID'){
				setCookie(name='#key#', value='', expires='NOW', domain='#cgi.REMOTE_ADDR#');

				// лишнее что пришло в кукисах можно сохран€ть в текстовый файл, в базу опасно.
				request.factoryService.getService('Clog').AddLogging(ssection='CCookie', type='event', description='Ќедопустимые символы в Cookie');
				// фаерволл
