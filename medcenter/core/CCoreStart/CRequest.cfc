component displayname="CRequest" {
	...
	// проверка урла запускаетс€ после фаервола и редиректа
	function CheckUrl() {
		...
		if (!check=='') {
			request.factoryService.getService('Clog').AddLogging(ssection='CRequest', type='event', description='Ќедопустимые символы в урле!');
	...
	// конструктор URL автоматическа€ добавка urlToken
